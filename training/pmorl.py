...
ckpt_params = torch.load(script_args.reward_model_name, map_location=f'cuda:{current_device}')
loading_cnt = 0
...
def add_preference_prompt_during_val(query, a, b, c):
    query = query.replace('\n<|assistant|>\n', '')
    query = query + f" {PREF_PROMPTS[a]} {PREF_PROMPTS[b]} {PREF_PROMPTS[c]}\n<|assistant|>\n"
    return query

def preference_prompt(query, i):
    return f"<|user|>\n{query} {PREF_PROMPTS[i]}\n<|assistant|>\n"
...
for steps, batch in tqdm(enumerate(ppo_trainer.dataloader)):
    ...
    for q, r in zip(batch["query"], batch["response"]):
        query, rm_index = check_preference(q)
        queries.append(query)
        rm_index_lst.append(rm_index)
    
    rewards_lst = []
    final_rewards = torch.zeros(len(rm_index_lst)).to(f'cuda:{current_device}')
    g_mean = 0
    for i in range(6):
        text = [preference_prompt(q, i) for q in queries]
        input_ids = tokenizer(text, max_length=script_args.max_length ,return_tensors="pt", padding=True, truncation=True).input_ids
        reward_outputs = reward_model(input_ids = input_ids.to(reward_model.device))[0]
        #reward_outputs = nn.functional.logsigmoid(reward_model(input_ids = input_ids.to(reward_model.device))[0])
        rewards = [torch.tensor(output[0]).float() - script_args.reward_baseline for output in reward_outputs]
        vmin, vmax, v_mean = get_reward_stats(rewards)
        for j in range(len(rewards)):
            rm = rewards[j]
            if i in rm_index_lst[j]:
                final_rewards[j] = final_rewards[j] + rm
                g_mean += v_mean
    final_rewards = [x / 3 for x in final_rewards]
    g_mean = g_mean / 3

    # Run PPO step
    stats = ppo_trainer.step(question_tensors, response_tensors, final_rewards)
    #ppo_trainer.log_stats(stats, batch, final_rewards)
    # First log stats during training
    wandb.log({"train_reward_mean": g_mean})
                ...
                for i, val_batch in tqdm(enumerate(val_dataloader)):
                    ...
                    if len(question_tensors) != 0:
                        ...
                        for q, r in zip(val_batch["original_query"], val_batch["response"]):
                            text = add_preference_prompt_during_val(q, random.randint(0,1), random.randint(2,3), random.randint(4,7)) + r 
                            texts.append(text)
